...
public class CustomListeners extends Utility implements ISuiteListener, IInvokedMethodListener2, ITestListener, IClassListener {
	...
	public void onStart(ISuite suite) {
		...
		try {
			...
			// If parameter is not found in XML then default browser will be picked from the config file
			if (browser == null) {
	...
	public void onFinish(ISuite suite) {
		...
		try {
			...
			// Tear down only perform when all suites has finished
	...
	public void beforeInvocation(IInvokedMethod method, ITestResult arg1, ITestContext arg2) {
		...
		testCaseID = tmp.substring(tmp.indexOf("SMITHS")).trim();
		...
		//Video Recording		
		try {
			MyScreenRecorder.startRecording(testCaseID);
		} catch (Exception e) {
	...
	public void onTestFailure(ITestResult result) {
		...
		// Log  Failed Test in HTML Report and End the Test
		HtmlReportUtil.testFail(GlobalUtil.getTestData().getTestId());
		//Update results in TL
	...
	// These methods are not used but required for Listener purpose
	/* (non- JavaDoc)
	...
	public void onFinish(ITestContext context) {
		...
		try {
			...
			moveFile("./ExecutionReports/Videos/"+testCaseID+""+mp4+"", "./ExecutionReports/ExeVideo/"+testCaseID+""+mp4+"", ".mp4");
			Utility.delDirectory(new File("./ExecutionReports/Videos/"+testCaseID+""+mp4+""));
